...
import { updateUserInfo, updateUserPhotoUrl } from '../../api/UserService';
import { toast } from 'react-toastify';
import { uploadImage } from '../../Firebase/service';
import { createFileFromBlob, fetchBlobFromUrl } from '../../Helpers/CommonMethod';
...
const UserEditForm = () => {
  ...
  const [newUserInfo, setNewUserInfo] = useState<UserProfile>(user);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await updateUserInfo(newUserInfo.id, newUserInfo)
      .then((res) => {
        if (res) {
    ...
    console.log(newUserInfo); // Handle form submission
  };
  ...
  const handleOnSubmit = async (event: any) => {
    event.preventDefault();
    if(fileRef.current && previewImage){
      let finalFileUrl = "";
      ...
      await updateUserPhotoUrl(user.id, finalFileUrl);

      setNewUserInfo((prevData) => ({
        ...prevData,
        'photoUrl': finalFileUrl, // Dynamically update the field based on name
      }));
  ...
  useEffect(() => {
    // console.log('fileUrl: ' + previewImage?.url);
  }, [previewImage]);
                  ...
                  <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                    ...
                    <div className="w-full sm:w-1/2">
                      ...
                      <input
                        ...
                        name="dob"
                        id="dob"
                        value={formatDate(newUserInfo.dob)}
                        onChange={handleChange}
                ...
                <form action="#">
                  <div className="mb-4 flex items-center gap-3">
                    <div className="h-14 w-14 rounded-full">
                      <img src={newUserInfo.photoUrl} alt="User" />
                    </div>
                    <div>
